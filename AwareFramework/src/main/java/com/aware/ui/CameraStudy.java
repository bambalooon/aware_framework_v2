...
public class CameraStudy extends Aware_Activity implements PreviewCallback, AutoFocusCallback {
    ...
    public void onPreviewFrame(byte[] data, Camera camera) {
        ...
        if( result != 0 ) {
            ...
            final String study_url = scanned;
            if( Aware.getSetting(getApplicationContext(), "study_id" ).length() > 0 ) {
				...
				builder.setMessage("You are already enrolled in a study. Are you sure you want to quit the previous study and enroll on this one?");
				builder.setNegativeButton("Cancel", new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
						dialog.dismiss();
						if( Aware.DEBUG ) Log.d(Aware.TAG, "User canceled joining study...");
						finish();
						return;
					}
				});
				builder.setPositiveButton("Check study", new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
						dialog.dismiss(); //carry on!
						if( Aware.DEBUG ) Log.d(Aware.TAG, "User is joining new study...");
						new StudyData().execute(study_url);
					}
				});
				builder.create().show();
			} else {
    ...
    private class StudyData extends AsyncTask<String, Void, JSONObject> {
		...
		protected JSONObject doInBackground(String... params) {
			...
			if( request != null && request.getStatusLine().getStatusCode() == 200 ) {
				try {
					JSONObject study_data = new JSONObject(EntityUtils.toString(request.getEntity()));
					return study_data;
		...
		protected void onPostExecute(JSONObject result) {
			...
			if( result == null ) {
				...
				builder.setMessage("This study is no longer ongoing.");
			} else {
				AlertDialog.Builder builder = new AlertDialog.Builder(CameraStudy.this);
	            builder.setPositiveButton("Sign up!", new DialogInterface.OnClickListener() {
	                ...
	                public void onClick(DialogInterface dialog, int which) {
	                    ...
	                    if( Aware.DEBUG ) {
	                        Toast.makeText(getApplicationContext(), "Joining: " + study_url, Toast.LENGTH_LONG).show();
	                    }
	                    finish();
	                }
	            });
