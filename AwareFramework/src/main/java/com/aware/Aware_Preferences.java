    ...
    protected void onResume() {
    	...
    	if( Aware.getSetting(getApplicationContext(), "study_id").length() == 0 || ! isAccessibilityServiceActive()) {
    		...
    		if( ! isAccessibilityServiceActive() ) {
                ...
                startActivity(accessibilitySettings);
                Toast.makeText(getApplicationContext(), "Please activate AWARE here.", Toast.LENGTH_LONG);
            }
    	}
    }
    ...
    public static class StudyConfig extends IntentService {
    	...
    	protected void onHandleIntent(Intent intent) {
			...
			if( study_url.startsWith("https://api.awareframework.com/") ) {
				...
				try {
					JSONArray configs_study = new JSONArray(EntityUtils.toString(answer.getEntity()));
